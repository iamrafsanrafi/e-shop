...
const ProductLayout2 = ({ discountTag, discountPercent, type, title, images, rating = 1, totalRatings, previousPrice, price, tags, stock, id }) => {
    ...
    if (window.innerWidth > 850 && title.length > 25) {
        formattedTitle = `${title.slice(0, 25)}...`;
    }
    else if(window.innerWidth <= 850 && title.length > 17) {
        formattedTitle = `${title.slice(0, 17)}...`;
    }
    ...
    return (
        <Link to={`/product-details/${id}`} className="p-10 block group cursor-pointer rounded-[10px] max-w-[464px] bg-[#EaEAEA] hover:bg-white transition duration-200 relative">
            <div className="relative bg-amber-200">
                ...
                {/* Discount Tag */}
                {discountTag && (
                    <div className="absolute top-0 right-0 w-[64px] h-[64px] xl:w-[100px] xl:h-[100px] text-white text-base sm:text-2xl font-['Poppins'] font-semibold leading-6  bg-[#FF624C] rounded-full flex items-center justify-center">{discountPercent}%</div>
                )}
            </div>
            <div className="mt-10">
                <p className="text-sm font-['Montserrat'] leading-5 text-[#303030] tracking-[4px] uppercase">{type}</p>

                <h3 className="text-[#303030] font-['Poppins'] text-lg sm:text-xl font-semibold leading-[30px]  mt-4 group-hover:text-[#FF624C] group-hover:underline decoration-2 transition duration-100">{formattedTitle}</h3>
