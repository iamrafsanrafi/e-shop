...
const ProductLayout = ({ discountTag = false, images, id, discountPercent, type, title, rating = 1, totalRatings, price, previousPrice, tags }) => {
    ...
    return (
        <div className="py-3 px-4 sm:p-6 group border border-[#C3C3C3] rounded-[10px] max-w-[285px] h-[490px] sm:h-[518px] relative">
            <div className="relative">
                <img className="w-[90%] mx-auto sm:w-full" loading="lazy" src={images[0]} alt="product image" />
            ...
            <Link to={`/product-details/${id}`} className="mt-10 block cursor-pointer">
                ...
                <h3 className="text-[#303030] font-['Poppins'] text-base sm:text-xl font-semibold leading-[30px] max-w-[237px] mt-4 group-hover:text-[#FF624C] group-hover:underline decoration-2 transition duration-100">{title.slice(0, 35)}...</h3>
                ...
                <div className="flex items-baseline-last gap-2">
                    <p className={`${discountTag ? "text-[#FF624C]" : "text-[#303030]"} font-['Poppins'] text-xl sm:text-2xl font-semibold leading-[30px] mt-5`}>${price}</p>
